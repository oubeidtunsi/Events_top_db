...
class AuthService:
    ...
    def _normalize_gender(value):
        """Converte i codici dell'app ('M','F','O') nei valori attesi dal DB.

        Valori DB ammessi: 'male', 'female', 'non-binary', 'other', 'guest'.
        Qualsiasi valore già corretto (es. 'guest') viene restituito invariato.
        """
        if not value:
            return None
        mapped = AuthService._GENDER_MAP.get(value.upper())
